...
public final class AdsbDemodulator {
    ...
    public RawMessage nextMessage() throws IOException {
        ...
        while (condition) {
            ...
            if (beforeP < sumP && sumPNext < sumP && sumP >= 2 * sumV) {
                ...
                if (Byte.toUnsignedInt(bit5) == 17) {
                    ...
                    for (int i = 0; i < 14; i++) {
                        b = "";
                        for (int j = 0; j < 8; j++) {
                            if (window.get(80 + (i*80) + 10 * j) >= window.get(85 + (80 * i) + 10 * j)) {
                                b += '1';
                            } else {
                                b += '0';
                            }
                        }
                        number = Integer.parseInt(b, 2);
                        table[i] = (byte) number;
                    }
                    // We need to check if crc = 0
                    // this method correctly gets the first message, good sign!! timeStamp = 63700
                    checker = RawMessage.of(timeStamp, table);
                    if (checker != null){
                        window.advanceBy(window.size()-1);
            ...
            if(!window.isFull()){
                lastWindowCounter++;
                if(window.size() == lastWindowCounter){
                    condition = false;
                }
            }
        }
